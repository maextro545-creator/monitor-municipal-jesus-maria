export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'news') {
      ...
      db.news = db.news.filter(item => item.url !== id);
      if (db.news.length < originalLength) updated = true;
    } else if (type === 'youtube') {
      const originalLength = db.youtube.length;
      db.youtube = db.youtube.filter(item => item.video_id !== id);
      if (db.youtube.length < originalLength) updated = true;
    } else if (type === 'twitter') {
      const originalLength = db.twitter.length;
      db.twitter = db.twitter.filter(item => item.url !== id);
      if (db.twitter.length < originalLength) updated = true;
    }
    ...
    await commitMultipleFiles(filesToCommit, `chore: eliminar item manual (${type}: ${id})`, owner, repo);
